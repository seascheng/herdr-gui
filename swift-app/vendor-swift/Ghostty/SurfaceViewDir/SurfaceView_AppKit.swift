...
extension Ghostty {
    ...
    class SurfaceView: OSView, ObservableObject, Identifiable {
        ...
        /// HERDR-GUI: invoked on renderer UPDATE_FRAME_END (renderer thread);
        /// drives the presentation-repair loop for the cmux fork.
        var rendererActivityHandler: (() -> Void)?
        ...
        deinit {
            ...
            // HERDR-GUI: UNUserNotificationCenter requires a bundle proxy,
            // absent in bare binaries; we post no user notifications.
        ...
        override func mouseMoved(with event: NSEvent) {
            ...
            // HERDR-GUI: focus-follows-mouse requires the upstream
            // BaseTerminalController; not applicable to our shell.
        }
        ...
        override func scrollWheel(with event: NSEvent) {
            // HERDR-MIRROR: the surface mirrors herdr's full app frame; wheel
            // events must be encoded as mouse input and sent to herdr (the
            // TUI owns pane scrolling/history), exactly like upstream.
            guard let surfaceModel else { return }
                ...
                if keySequence.isEmpty,
                   ...
                   bindingFlags.contains(.consumed) {
                    if let appDelegate = NSApp.delegate as? AppDelegate,
                       appDelegate.performGhosttyBindingMenuKeyEquivalent(event) {
                        return true
                    }
                }
        ...
        @IBAction func copy(_ sender: Any?) {
            ...
            if !ghostty_surface_binding_action(surface, action, UInt(action.lengthOfBytes(using: .utf8))) {
                HerdrLog.warning("action failed action=\(action)")
            }
        }
